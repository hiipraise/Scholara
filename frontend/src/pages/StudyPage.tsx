...
export default function StudyPage() {
  ...
  // Determine which term to fetch exams for: current or selected past term
  const currentTermKey = `${user?.level}|${user?.semester}`;
  const pastTerms = (cycleHistory || []).filter(
    (t) => `${t.level}|${t.semester}` !== currentTermKey,
  );
  const activePastTermKey =
    selectedPastTerm ||
    (pastTerms[0] ? `${pastTerms[0].level}|${pastTerms[0].semester}` : "");
  const selectedPast = pastTerms.find(
    (t) => `${t.level}|${t.semester}` === activePastTermKey,
  );
  ...
  const examTermKey =
    cycleTab === "current"
      ...
      : selectedPastTerm ||
        (pastTerms[0] ? `${pastTerms[0].level}|${pastTerms[0].semester}` : "");
  const [examLevel, examSemesterStr] = (examTermKey || "").split("|");
  const examSemester = examSemesterStr ? Number(examSemesterStr) : 1;

  const { data: examSlots, isLoading: examLoading } = useQuery({
    queryKey: ["exam-timetable", examLevel, examSemester, cycleTab],
    queryFn: () =>
      adminApi
        .getExamTimetable(
          examLevel || user?.level,
          examSemester || user?.semester,
          cycleTab === "past",
        )
        .then((r) => r.data),
    enabled: !!examLevel || !!user?.level,
  });
  ...
  const currentStudyDay = getCurrentStudyDay(
    activeCalendar?.lectures_start_date ?? null,
  );

  // Group exams by date
  const examsByDate =
    examSlots?.reduce<Record<string, ExamSlot[]>>((acc, slot) => {
  ...
  const upcomingExams = sortedExamDates.filter(
    (d) => !isPast(parseISO(d)) || isToday(parseISO(d)),
  );
    ...
    <div className="space-y-8 pb-12">
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          {cycleTab === "past" && (
            ...
            >
              {pastTerms.map((t) => (
                <option
                  key={`${t.level}|${t.semester}`}
                  value={`${t.level}|${t.semester}`}
                >
        ...
        {cycleLoading ? (
          <div className="grid gap-3 sm:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="card h-36 shimmer-bg" />
            ))}
          </div>
        ) : (
          <>
                      ...
                      {displayedCycle?.map((day) => (
                        ...
                        >
                          <div className="w-16 shrink-0">
                            <div className="text-cream-200/50 text-xs font-semibold">
                              Day {day.day_number}
                            </div>
                            {day.day_number === currentStudyDay && (
                              <div className="text-accent-sage text-[10px]">
                                Today
                              </div>
                            )}
                          </div>
                    ...
                    <div className="flex-1 space-y-2">
                      {slots.map((slot) => (
                        ...
                        >
                          ...
                          <div>
                            <span className="text-cream-200 font-semibold text-sm">
                              {slot.course_code}
                            </span>
                            <span className="text-cream-200/45 text-xs ml-2">
                              {slot.course_title}
                            </span>
                          </div>
                          {isTodayDate && (
      ...
      {activeCalendar && (
        <motion.div
          ...
          className="card p-5"
        >
          <h3 className="text-cream-200/60 text-sm font-semibold mb-4">
            Academic Calendar
          </h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              {
                label: "School Resumed",
                value: format(
                  parseISO(activeCalendar.school_resume_date),
                  "MMM d, yyyy",
                ),
              },
              {
                ...
                value: format(
                  parseISO(activeCalendar.lectures_start_date),
                  "MMM d, yyyy",
                ),
              },
              ...(activeCalendar.semester_end_date
                ? [
                    {
                      label: "Semester Ends",
                      value: format(
                        parseISO(activeCalendar.semester_end_date),
                        "MMM d, yyyy",
                      ),
                    },
                  ]
              ...
              <div key={label} className="bg-cream-200/4 rounded-xl p-3">
                <div className="text-cream-200/35 text-[10px] uppercase tracking-wider mb-1">
                  {label}
                </div>
                <div className="text-cream-200/80 text-sm font-semibold">
                  {value}
                </div>
              </div>
            ))}
